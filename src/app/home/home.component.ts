...
export class HomeComponent implements OnInit, OnDestroy {
            ...
            response => {
                if (response.status == 200) {
                    if (JSON.parse(response._body).length != 0) {
                        ...
                        console.log(this.request,"request from kart");
    ...
    callReconnect(
        ...
        sessionStorage.setItem('endPoint', endPoint);
        sessionStorage.setItem('endPointId', endPointId);
        sessionStorage.setItem('endpointName', endpointName);
        sessionStorage.setItem('endpointLocation', endpointLocation);
    ...
    viewPACS(req: Requests) {
        let ref = this.dialog.open(ViewPacsDialogComponent, {
            width: '432px',
            data: req
        });
        ref.afterClosed().subscribe(result => {
            if (result) {
                if (req.status == 'DeferredState') {
                    this._router.navigate([
                        'viewpacs/' + req.mrn + '/' + req.id
                    ]);
                } else {
                    this._specialistRequestService
                        .setSpecialistRequest(
                            req.id,
                            'Deferred',
                            'Please wait, I will join shortly.'
                        )
                        .subscribe(
                            response => {
                                this._router.navigate([
                ...
                sessionStorage.setItem('facilityName', req.facilityName);
                sessionStorage.setItem('specialityName', req.specialityName);
                sessionStorage.setItem(
                    'reasonForRequest',
                    req.reasonForRequest
                );
                sessionStorage.setItem('endPoint', req.endPoint);
                sessionStorage.setItem('endPointId', req.endPointId);
            } else {
            }
        });
    }
